...
int main(int argc, char** argv){
	...
	for(int i = 0; i < numObs; ++i){
		...
		problem.AddResidualBlock(lambdaError, new ceres::HuberLoss(0.015), rotAngleAxis, lambdas);
		...
		// Add a residual block to the problem
		problem.AddResidualBlock(lambdaRegularizer, new ceres::HuberLoss(0.05), lambdas);
